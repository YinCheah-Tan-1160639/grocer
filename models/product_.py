...
class Product:
    ...
    def get_all_vegetables(self, sort_by: str = 'name') -> list:
        ...
        @return A list of all vegetables sorted by name.
        """
        try:
            all_veggies = [] # create an empty list to hold all vegetables

            # Add unit to each vegetable and combine all lists
            all_veggies += [{'id': veg['id'], 'name': veg['name'], 'price': veg['price'], 'available': veg['available'], 'unit': veg['unit']} for veg in self.veggies_kg]
            all_veggies += [{'id': veg['id'], 'name': veg['name'], 'price': veg['price'], 'available': veg['available'], 'unit': veg['unit']} for veg in self.veggies_pack]
            all_veggies += [{'id': veg['id'], 'name': veg['name'], 'price': veg['price'], 'available': veg['available'], 'unit': veg['unit']} for veg in self.veggies_each]
            return sorted(all_veggies, key=lambda x: x[sort_by]) # sort by name
    ...
    def get_available_vegetables(self, sort_by: str = 'name') -> list:
        ...
        @return A list of all available vegetables sorted by name.
        """
        try:
            # Get all sorted vegetables
            all_veggies = self.get_all_vegetables(sort_by)
            # Filter for available vegetables
    ...
    def get_all_premade_boxes(self) -> list:
        ...
        @return A list of all premade boxes sorted by size.
        """
        # Assign a value to each size
        sort_box = {"Small": 1, "Medium": 2, "Large": 3}
        try:
            return sorted(self.premade_boxes, key=lambda box: sort_box[box['size']]) # sort by size
        except Exception as e:
